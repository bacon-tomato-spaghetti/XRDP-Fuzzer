...
unsigned char *g_inbuf;
unsigned char g_outbuf[0x100000];
...
/*****************************************************************************/
int trans_tcp_recv(struct trans* self, char* buf, int len) {
    memcpy(buf, g_inbuf, len);
    return len;
}

int trans_tcp_send(struct trans* self, const char* buf, int len) {
    memcpy(g_outbuf, buf, len);
    return len;
}

int trans_tcp_can_recv(struct trans* self, int dd, int len) {
    return 1;
}
